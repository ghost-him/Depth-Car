...
def lane():
    ...
    # 加载模型
    save_path = "../model/model_infer0726_1580_4_150"
    ...
    while True:
        ...
        if ret == True:
            ...
            if vel == vel_dic["start"]:
                angle = int((angle - 1500) * ang_dic["start"] + 1500)
                # 转向角过大时，判定进入U形弯
                if angle < 1200 or angle > 1800:
                    vel = vel_dic["Uturn"]
            # 2. U形弯
            elif vel == vel_dic["Uturn"]:
                # 减速过弯，转向角按比例缩放
                angle = int((angle - 1500) * ang_dic["Uturn"] + 1500)
                # if angle < 1000:
                #     angle = 1000
                # if angle > 2000:
                #     angle = 2000
            ...
            # 4. 人行横道--上坡 过渡段
                ...
                angle = int((angle - 1500) * ang_dic["c-2-s"] + 1500)
                if angle < 1300:
                    angle = 1300
                if angle > 1700:
                    angle = 1700
            ...
            # 6. 上坡--限速区 过渡段
                ...
                # 低速行驶，转向角成比例减小，并设置上下限，防止车头剧烈摆动
                angle = int((angle - 1500) * ang_dic["s-2-l"] + 1500)
                if angle < 500:
                    angle = 500
                if angle > 2500:
                    angle = 2500
            ...
            # 7. 限速区
                ...
                # 低速行驶，转向角成比例减小，并设置上下限，防止车头剧烈摆动
                angle = int((angle - 1500) * ang_dic["limit"] + 1500)
                if angle < 1000:
                    angle = 1000
                if angle > 2000:
                    angle = 2000
            # 8. 解除限速，进入S弯和环岛
                ...
                angle = int((angle - 1500) * ang_dic["cancl"] + 1500)
                if angle < 1000:
                    angle = 1000
                if angle > 2000:
                    angle = 2000
            ...
            # 9. 减速看红灯
                ...
                # 低速行驶，转向角成比例减小，并设置上下限，防止车头剧烈摆动
                angle = int((angle - 1500) * ang_dic["slowd"] + 1500)
                if angle < 1000:
                    angle = 1000
                if angle > 2000:
                    angle = 2000
            ...
            # 11. 绿灯行
                ...
                # 转向角成比例增大，应对直角弯
                angle = int((angle - 1500) * ang_dic["green"] + 1500)
                if angle < 200:
                    angle = 200
                if angle > 2800:
                    angle = 2800
            ...
            cv2.putText(frame, f'vel: {vel}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)
            cv2.putText(frame, f'ang: {angle}', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            if conf >= 0.80:
                                ...
                                cv2.putText(image, str(conf), (coor[0], coor[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2)
                                cv2.putText(image, label, (coor[0], coor[3] + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255),
                                            2)
                                ...
                                if label == 'crossing':
                                    # 人行道
                                    if area >= area_dic["cross"]:
                                        print('人行道')
                                        vel = vel_dic["cross"]
                                        q.put(vel)
                                elif label == 'uphill_slope':
                                    # 上坡
                                    if area >= area_dic["slope"]:
                                        print('上坡,', area)
                                        vel = vel_dic["slope"]
                                        q.put(vel)
                                elif label == 'limit_10':
                                    # 限速
                                    if area >= area_dic["limit"]:
                                        print('限速')
                                        vel = vel_dic["limit"]
                                        q.put(vel)
                                elif label == 'cancel_10':
                                    # 限速解除
                                    if area >= area_dic["cancl"]:
                                        print('解除限速')
                                        vel = vel_dic["cancl"]
                                        q.put(vel)
                                    ...
                                    # 红灯
                                    if area_dic["redmax"] >= area >= area_dic["redmin"]:
                                        print('红灯,', area)
                                        vel = vel_dic["redlt"]
                                        q.put(vel)
                                elif label == 'paper_greend':
                                    # 绿灯
                                    if area >= area_dic["green"]:
                                        print('绿灯')
                                        vel = vel_dic["green"]
                                        q.put(vel)
                            del coor
